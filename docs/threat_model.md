...
| **TM-08** | **Information Disclosure / SSRF** | **Tracker overlay proxies**<br>Weather, markets, currencies, flights, news, ships, webcams, and city temperatures call third-party APIs. A crafted request could try to forward user text or open an internal URL. | **Medium** | Overlay routes are read-only GET proxies with fixed provider URLs. They do not forward search text. News uses a fixed highlight query. Webcam URLs must be `http`/`https`. Shared 60s cache reduces repeat fetches. | **Mitigated** |
